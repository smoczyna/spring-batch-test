...
//    WholesaleBookingProcessorHelper processingHelper;
    ...
    @Bean
    Flow initialFlow(@Qualifier("checkIfSourceFilesExist") Step checkIfSourceFilesExist,
        ...
        return new FlowBuilder<Flow>("initialFlow")
                ...
                .next(updateBookingDatesStep)
                .next(readOffsetDataStep)
                .build();        
    }
    
    @Bean
    Flow billedProcessingFlow(@Qualifier("billedFilePartitionStep") Step billedFilePartitionStep) {
        return new FlowBuilder<Flow>("billedProcessingFlow").start(billedFilePartitionStep).build();
    }
    
    @Bean
    Flow unbilledProcessingFlow(@Qualifier("unbilledFilePartitionStep") Step unbilledFilePartitionStep) {
        return new FlowBuilder<Flow>("unbilledProcessingFlow").start(unbilledFilePartitionStep).build();
    }
    
    @Bean
    Flow adminFeesProcessingFlow(@Qualifier("adminFeesFilePartitionStep") Step adminFeesFilePartitionStep) {
        return new FlowBuilder<Flow>("adminFeesProcessingFlow").start(adminFeesFilePartitionStep).build();
    }
    
    @Bean
    Flow splitFlow(@Qualifier("billedProcessingFlow") Flow billedProcessingFlow,
                   @Qualifier("unbilledProcessingFlow") Flow unbilledProcessingFlow,
                   @Qualifier("adminFeesProcessingFlow") Flow adminFeesProcessingFlow) {
        return new FlowBuilder<Flow>("splitFlow")
                .start(adminFeesProcessingFlow)
                .split(new SimpleAsyncTaskExecutor())
                .add(billedProcessingFlow, unbilledProcessingFlow)
                .build();
    }
    ...
    Job bookingAggregateJob(JobExecutionListener bookingFileJobListener,
                            ...
                            @Qualifier("initialFlow") Flow initialFlow,
